...
export const serverHydration: Hydrate<APIServer, HydratedServer> = {
  ...
  functions: {
    ...
    icon: (server, ctx) => (server.icon ? new File(ctx as Client, server.icon) : undefined),
    banner: (server, ctx) => (server.banner ? new File(ctx as Client, server.banner) : undefined),
